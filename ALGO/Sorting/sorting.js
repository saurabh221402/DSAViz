...
let barCount = document.getElementById('barsCount');
let n = barCount.value;
let speedManager = document.getElementById('speedManager');
let delay = 375 - speedManager.value;
...
barCount.oninput = () => {
    document.querySelector('.barCountValue').innerHTML = `Bars: ${barCount.value}`;
    generateRandomArray();
  };
  speedManager.oninput = () => {
    delay = 375 - speedManager.value;
  };
  
  document.getElementById('generateBars').addEventListener('click', generateRandomArray);
  document.getElementById('startSorting').addEventListener('click', () => {
    ...
    else if (type == 'merge'){ alert("working on it !"); return ;}
    else if (type == 'quick'){ alert("working on it !"); return ;}
  });
  document.getElementById('resetSorting').addEventListener('click', () => {
    isStopped = true;
    isPaused = false;
    document.getElementById('pauseSorting').innerHTML = 'Pause';
  ...
  document.getElementById('pauseSorting').addEventListener('click', () => {
    ...
    if (!isStopped) {
      ...
      if (isPaused) {
        ...
        document.getElementById('pauseSorting').innerHTML = 'Resume';
        isPaused = true;
      }
    }
  });